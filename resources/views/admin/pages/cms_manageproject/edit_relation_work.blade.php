...
@section('content')
  ...
  @php
    ...
    $userWorkArray = explode(',', $cmsRelationWork->user_work);
    $departmentWorkArray = explode(',', $cmsRelationWork->department_user_work);
  @endphp
  ...
  <section class="content">
    ...
    <div class="box box-primary">
      ...
      <form role="form" action="{{ route('admin.update_relation_work') }}" method="POST" enctype='multipart/form-data'>
        @csrf
        <input type="hidden" value="{{ $cmsRelationWork->id }}" name="relation_work_id" id="relation_work_id">
        <div class="box-body">
          <!-- Custom Tabs -->
            ...
            <ul class="nav nav-tabs">
              <li class="active">
                <a href="#tab_1" data-toggle="tab">
                  <h5>Cập nhật công việc: <span style="color:red">{{ $cmsRelationWork->title }}</span></h5>
                </a>
              </li>
              <button type="submit" class="btn btn-danger pull-right btn-sm mg-5"  name="submit">Cập nhật</button>
            </ul>
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="title" placeholder="Nhập tên công việc"
                        value="{{ $cmsRelationWork->title }}" required>
                    </div>
                  </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input class="form-control" name="deadline" id="deadline" type="datetime-local" value="{{ $cmsRelationWork->deadline }}">
                    </div>
                  </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <textarea name="content" class="form-control" id="content_vi">{{ $cmsRelationWork->content }}</textarea>
                      </div>
                  </div>
				  ...
				  <div class="col-md-6">
                    <div class="form-group">
                      <label>Danh sách file đã đính kèm</label>
					    @foreach($files as $k => $file)
							<div class="form-group">
								  <div class="col-md-6" style="padding-left:0px;">
									  <input class="form-control" type="text" readonly value="<?= $file->name ?>" />
								  </div>
								  <div class="col-md-2" style="padding-left:0px;">
									  <input class="btn btn-danger form-control"  onclick="deleteFile(<?= $file->id?>)" value="Xóa"/>
								  </div>
							</div>
						@endforeach
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="status" class="form-control select2" id="status">
                        ...
                        @foreach ($status_work as $key => $value)
                          <option value="{{ $key }}"
                              {{ $cmsRelationWork->status == $key ? 'selected' : '' }}>
                              {{ $value }}
                          </option>
                        @endforeach
                      </select>
                    ...
                    <div class="form-group">
                       <label>@lang('Cấu hình công việc')</label>
                        <div class="form-control">
                          <input type="radio" name="is_public" value="0" class="check-all-congkhai" {{ $cmsRelationWork->is_public == 0 ? 'checked' : '' }}>
                          Cá nhân
                          <input type="radio" name="is_public" value="1" class="ml-15 check-all-congkhai" {{ $cmsRelationWork->is_public == 1 ? 'checked' : '' }}>
                          Công khai
                      </div>
                    </div>
                  </div>
                      ...
                      <div class="container_table custom-scroll table-responsive no-margin" style="max-height: 400px; overflow-y: scroll;">
                        ...
                        <table class="table table-striped table-bordered table-hover no-footer" width="100%" style="border: 1px solid #ccc !important">
                          ...
                          <tbody id="load-ajax" >
                            ...
                            <tr>
                              <td class="">
                                <input class="_check_all checkbox check_class" id="check_class_{{ $department->id }}" type="checkbox" name="department_user_work[]" value="{{ $department->id }}" 
                                {{ in_array($department->id, $departmentWorkArray) ? 'checked' : '' }}> <span>{{ $department->title }}</span>
                              </td>
                              <td class="">
                                <?php foreach($admins as $admin){ 
                                  if($admin->department_id == $department->id){
                                  ?>
                                  <input class="checkbox _check_all _check_relative _check_all_{{ $department->id }}" type="checkbox" name="user_work[]" value="{{ $admin->id }}" data-value="{{ $department->id }}" {{ in_array($admin->id, $userWorkArray) ? 'checked' : '' }}> <span>{{ $admin->name }}</span><br>
                                <?php } } ?>
                              </td>
                            </tr>
                            @endforeach
        ...
        <div class="box-footer">
          <a class="btn btn-success btn-sm" href="{{ route('admin.view_relation_work', ['id' => $cmsRelationWork->manage_work_id]) }}">
            <i class="fa fa-bars"></i> @lang('List')
          </a>
            <button type="submit" class="btn btn-danger pull-right btn-sm mg-5" name="submit" value="gui">Cập nhật</button>
...
<script>
  ...
  function deleteFile(id){
	  if(confirm("Bạn chắc chắn muốn xóa file này?")) {
		var f = "id=" + id ;
		var _url = "{{ route('admins.relation-work.delete-file') }}";
		$.ajax({
		  url: _url,
		  data: f,
		  cache: false,
