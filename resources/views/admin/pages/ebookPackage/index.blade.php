    ...
    <section class="content">
        ...
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">Danh sách khổ sách</h3>
            </div>
                    ...
                    <table class="table table-hover table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th>Giá (1 trang/ MB)</th>
                                <th>x% Giá sách giấy</th>
                                <th>Giá tối thiểu</th>
                                <th>Làm tròn</th>
                                <th>Trạng thái</th>
                                <th>@lang('Action')</th>
                            ...
                            @foreach ($rows as $key => $row)
                                <form action="{{ route(Request::segment(2) . '.destroy', $row->id) }}" method="POST"
                                    ...
                                    <tr class="valign-middle">
                                        <td>{{ $key + 1 }}</td>
                                        <td><?= $row->title ?></td>
                                        <td><?= $array_booktype[$row->book_type] ?></td>
                                        <td><?= $row->time ?> ngày</td>
                                        <td><?= $array_recipe[$row->recipe] ?></td>
                                        <td><?= $row->price ?></td>
                                        <td><?= $row->percent ?></td>
                                        <td><?= number_format($row->min_price) ?></td>
                                        <td><?= $array_rounding[$row->rounding] ?></td>
