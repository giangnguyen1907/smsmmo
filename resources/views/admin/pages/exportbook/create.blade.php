						...
						<table id="ListProduct" class="table table-thead-bordered table-nowrap table-align-middle">
							...
							<tbody>
								<?php 
									...
									if(session('exportbook')){ 
									foreach(session('exportbook') as $id=> $s_import){ $index++;
...
<script>
	...
	function deleteProduct(id){
		$.ajax({
      url: '{{ route('export_book.save_delete') }}',
      type: 'POST',
      data: {
          '_token': '{{ csrf_token() }}',
          'id': id,
	...
	function InfomationBook(){
		...
		if(id > 0){
			...
			$.ajax({
	      url: '{{ route('export_book.save_ajax') }}',
	      type: 'POST',
	      data: {
	          '_token': '{{ csrf_token() }}',
	          'id': id,
	...
	function updateQuantity(j,id){
		...
		$.ajax({
      url: '{{ route('export_book.update_ajax') }}',
      type: 'POST',
      data: {
          '_token': '{{ csrf_token() }}',
          'id': id,
