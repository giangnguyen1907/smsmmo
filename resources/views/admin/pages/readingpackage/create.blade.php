				  ...
				  <div class="col-md-6">
                      ...
                      <select name="category[]" id="category" multiple class="form-control select2" style="width: 100%">
                        ...
                        @foreach ($parents as $item)
                          @if ($item->parent_id == 0 || $item->parent_id == null)
                            <option value="{{ $item->id }}"  <?php if($item->id == old('category') ) echo 'selected'; ?> >
                              {{ $item->title->$locale }}</option>

                            @foreach ($parents as $sub)
                              @if ($item->id == $sub->parent_id)
                                <option value="{{ $sub->id }}"
                                  <?php if($sub->id == old('category') ) echo 'selected'; ?>>- - {{ $sub->title->$locale }}
                                </option>

                                @foreach ($parents as $sub_child)
                                  @if ($sub->id == $sub_child->parent_id)
                                    <option value="{{ $sub_child->id }}"
                                      <?php if($sub_child->id == old('category') ) echo 'selected'; ?>>- - - -
					...
					<div class="col-md-3">
						<div class="form-group">
							<div class="form-control">
								<label>
									<input type="checkbox" name="is_checkip" value="1">
					...
					<div class="col-md-3">
						<div class="form-group">
							<div class="form-control">
								<label>
									<input type="checkbox" name="is_audio" value="1">
									<small>Audio</small>
								</label>
							</div>
						</div>
